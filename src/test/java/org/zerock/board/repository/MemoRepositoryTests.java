...
public class MemoRepositoryTests {
    ...
    public void testSelect() {
        ...
        //select * from 테이블명 where bno=100;
        System.out.println("============ mno = 100 =============");
        if (result.isPresent()) {
            Memo memo = result.get();
            System.out.println(mno);
        }
        //findById(mno) 는 쿼리가 미리 실행됨. 결과 출력은 나중에
    ...
    public void testSelect2() {
        Long mno = 100L;
        Memo memo = memoRepository.getOne(mno); //getOne 은 현재 차단된 메서드(보안상)
        //@Transactional 필수, 변수가 호출될 때 쿼리가 실행된다.
        System.out.println("=============== Long mno = 100L ===================");
    ...
    public void testPageDefaults() {
        ...
        Sort sortAll = sort1.and(sort2); //내림차순 번호 & memoText 오름차순
    ...
    public void testQueryMethodWithPage() {
        ...
        //페이지 타입은 of를 이용해서 요청을 처리함, 0번에 10개를 mno를 기준으로 내림차순정렬을 매개값으로 전달
    ...
    @Commit //delete인 경우 auto commit이 안되기 때문에 commit을 넣어줘야함
    public void testDeleteQueryMethod() {
        //쿼리 메서드로 delete 처리를 하면 9번의 쿼리문이 전달되기 때문에 매우 비효율적! => @Query 를 사용해야 좋다!
